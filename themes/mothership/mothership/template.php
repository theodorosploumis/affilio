...
function mothership_breadcrumb($variables) {
  //add title to the breadcrumb
  // $variables['breadcrumb'][] = drupal_get_title();
  $breadcrumb = $variables['breadcrumb'];

  //@todo: this should be in a theme file ...
  if (!empty($breadcrumb)) {

    $output = '<nav rel="navigation" class="breadcrumb"><ul>';
  ...
  */
  ...
  if ( $hook == "html" ) {
    ...
    */
    ...
    if (theme_get_setting('mothership_css_contextual')) {
      drupal_add_css($path_mothership . '/css-drupalcore/contextual.css', array('group' => CSS_THEME, 'every_page' => TRUE, 'weight' => 0));
    }
    ...
    if (theme_get_setting('mothership_modernizr')) {
      drupal_add_js('//cdnjs.cloudflare.com/ajax/libs/modernizr/2.7.1/modernizr.min.js', 'external');
    }

    //---html5 fix
    $vars['html5iefix'] = '';
    if(theme_get_setting('mothership_html5') && theme_get_setting('mothership_html5_print')) {
      $vars['html5iefix'] .= '<!--[if lt IE 9]>';
      $vars['html5iefix'] .= '<script src="' . $path_mothership . '/js/html5shiv-printshiv.min.js"></script>';
      $vars['html5iefix'] .= '<![endif]-->';
    } elseif (theme_get_setting('mothership_html5')) {
      $vars['html5iefix'] .= '<!--[if lt IE 9]>';
      $vars['html5iefix'] .= '<script src="' . $path_mothership . '/js/html5shiv.min.js"></script>';
      $vars['html5iefix'] .= '<![endif]-->';
    }
